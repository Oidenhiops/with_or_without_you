...
public class ManagementSlashSkill : MonoBehaviour, ManagementCharacterSkills.ISkill
{
    ...
    {
        transform.position += characterMakeDamage.characterInfo.characterAttack.GetDirectionAttack().transform.forward;
        transform.rotation = characterMakeDamage.characterInfo.characterAttack.GetDirectionAttack().transform.rotation;
        transform.SetParent(null);
        rb.AddForce(characterMakeDamage.characterInfo.characterAttack.GetDirectionAttack().transform.forward * speed);
        GetComponent<ManagementInstanceAttack>().instanceAttackInfo.objectMakeDamage = character.gameObject;
        GetComponent<ManagementInstanceAttack>().instanceAttackInfo.damage = GetStatistics(Character.TypeStatistics.Atk, currentStatistics).baseValue;
        Destroy(rb, 2);
